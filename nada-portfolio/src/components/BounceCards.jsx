...
export default function BounceCards({
  ...
  // Parse transformStyles into { x, rotation } for GSAP
  const parsedTransforms = transformStyles.map(str => {
    const rotMatch = str.match(/rotate\(([-0-9.]+)deg\)/);
    const xMatch = str.match(/translateX\(([-0-9.]+)px\)/);
    return {
  ...
  useEffect(() => {
    ...
    // Set initial x and rotation from transformStyles
    parsedTransforms.forEach((t, i) => {
      gsap.set(q(`.card-${i}`), { x: t.x, rotation: t.rotation });
    });
  }, [animationStagger, easeType, animationDelay, parsedTransforms]);
  ...
  const pushSiblings = hoveredIdx => {
    ...
    images.forEach((_, i) => {
      ...
      if (i === hoveredIdx) {
        ...
        const offsetX = i < hoveredIdx ? -200 : 160;
        const distance = Math.abs(hoveredIdx - i);
        const delay = distance * 0.05;
        gsap.to(target, {
          x: parsedTransforms[i].x + offsetX,
          rotation: parsedTransforms[i].rotation,
          duration: 0.4,
          ease: 'back.out(1.4)',
          delay,
          overwrite: 'auto'
